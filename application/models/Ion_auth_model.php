...
*/
	...
	public function salesCode()
	{
		...
		if($this->db->where('customer_code', $code)->count_all_results($this->tables['users']) > 0)
		{
			salesCode();
		}else{
			return $code;
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->set_error(implode("<br>", $this->db->error()));
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (float)$group_id, $this->join['users'] => $user_id)))
			{
